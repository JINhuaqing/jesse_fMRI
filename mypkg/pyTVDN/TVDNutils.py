...
# Reconstruct Xmat from results with SC
# note SC is a symmetric matrix, so there is no need to consider complex number
def ReconXmatSC(ecpts, ndXmat, nXmat, eigVecs, Ymat, tStep, is_full=False):
    """
    Input:
        ecpts: Estimated change points,
    ...
    for  itr in range(numchgfull-1):
        ...
        Ycur = ndXmat[:, lower:upper]
        Xcur = nXmat[:, lower:upper]
        lams = np.diag(Ycur@Xcur.T)/np.diag(Xcur@Xcur.T)
        ResegS[itr, :] = lams
    ...
    if is_full:
        ReDict = edict()
        ReDict.EstXmatReal = detrend(EstXmat)
        ReDict.EstXmatRealOrg = EstXmat
        ReDict.LamMs = LamMs
        return ReDict
    else:
        return detrend(EstXmat)
