...
class TVDNSCDetect:
    def __init__(self, Ymat=None, dXmat=None, Xmat=None, eigVals=None, eigVecs=None, smoothType="Bspline", dataType=None, saveDir=None, showProgress=True, **paras):
        """
        Input:
        ...
        assert np.abs(eigVals.imag).sum() == 0, "all eig vals should be non-negative"
    ...
    def GetNewData(self):
        ...
        eigVecsInv = eigVecs.T # SC is a real symmetric matrix 
        ...
        self.nXmat = eigVecsInv[:r, :].dot(self.Xmat)
        self.ndXmat = eigVecsInv[:r, :].dot(self.dXmat)
        self.midRes = edict({"nXmat": self.nXmat, "ndXmat":self.ndXmat, "eigVecs":eigVecs, 
                            "eigVals":eigVals, "r":r})
    ...
    def GetRecResCur(self):
        ...
        if self.RecYmatAll is not None:
            ...
            if RecYmatAllPath.exists():
                ...
                self.RecResCur = ReconXmatSC(finalRes.chgMat[numchg-1, :numchg], ndXmat, nXmat, eigVecs, self.nYmat, tStep, is_full=True) 
        else:
            MaxM = self.paras.MaxM
            r = self.paras.r
            finalRes = self.finalRes
    ...
    def GetRecYmats(self):
        ...
        #            print(f"Current number of change point is {numchg}.")
                    if numchg == 0:
                        RecResCur = ReconXmatSC([], ndXmat, nXmat, eigVecs, self.nYmat, tStep, is_full=True) 
                    else:
                        RecResCur = ReconXmatSC(finalRes.chgMat[numchg-1, :numchg], ndXmat, nXmat, eigVecs, self.nYmat, tStep, is_full=True) 
